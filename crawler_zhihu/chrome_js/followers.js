...
function showresult() {
    ...
    if (!showtable) {
        ...
        rsdiv.html("编号,用户名,关注者,提问,回答,赞同");
        for (i in result) {
            rsdiv.append("<br/>" + (parseInt(i) + 1) + ",<a href='/people/" + result[i].id + "/' target='_blank'>" + result[i].name + "</a>," + result[i].follower + "," + result[i].ask + "," + result[i].answer + "," + result[i].agree);
        }
    }
}
...
function showratio() {
    ...
    //sortresult();  uncomment this if you want to sort result.
    showresult();
    usercursor++;
    loaduser();
}
...
$("#switchshowtable").click(function () { showtable = !showtable; $(this).val(showtable ? "改为表格分隔" : "改为逗号显示"); showresult(); });
