...
function showresult() {
    ...
    var rsdiv = $("#result");
    if (!showtable) {
        var tablehtm = "<table border='1' cellpadding='2' style='border-collapse: collapse;'><tr><td>编号</td><td>用户名</td></tr>";
        for (i in result) {
            tablehtm += "<tr><td>" + (parseInt(i) + 1) + "</td><td><a href='/people/" + result[i].id + "/' target='_blank'>" + result[i].name + "</td></tr>";
        }
        tablehtm += "</table>";
        rsdiv.html(tablehtm);
    }
        ...
        rsdiv.html("编号,用户名");
        for (i in result) {
            rsdiv.append("<br/>" + (parseInt(i) + 1) + ",<a href='/people/" + result[i].id + "/' target='_blank'>" + result[i].name);
        }
    }
}
