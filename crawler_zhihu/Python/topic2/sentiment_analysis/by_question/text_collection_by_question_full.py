...
def parse_comments(question_id, userid, data_aid):
    ...
    # 写入对应用户回答的评论
    global fp2
    fp2 = codecs.open(r'/Users/white/github/Undergraduate-Innovation-Program/crawler_zhihu/Python/topic2/sentiment_analysis/by_question/'+ str(question_id)+ '(fullcomment).txt', 'w', 'utf-8')
    for j in range(len(people_name)):
        fp2.write( people_name[j].strip() + '\t' + comment_text[j].strip() )
        fp2.write('\n')
...
def getAnswerer_content_comment(question_id):
    ...
    for i in range(len(answerer_id)):
        fp1 = codecs.open(r'/Users/white/github/Undergraduate-Innovation-Program/crawler_zhihu/Python/topic2/sentiment_analysis/by_question/'+ str(question_id)+ '(full).txt', 'w', 'utf-8')
        fp1.write(content[i])
        fp1.write('\n')
